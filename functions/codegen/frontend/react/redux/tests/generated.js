//user reducers types
export const SET_AUTH = "SET_AUTH";
export const SET_UNAUTH = "SET_UNAUTH";
export const SET_USER = "SET_USER";
export const LOADING_USER = "LOADING_USER";
export const SIGNUP_USER = "SIGNUP_USER";

// UI reducer types
export const LOADING_UI = "LOADING_UI";
export const STOP_LOADING_UI = "STOP_LOADING_UI";
export const CLEAR_ERRORS = "CLEAR_ERRORS";
export const SET_ERRORS = "SET_ERRORS";
export const SET_MESSAGES = "SET_MESSAGES";
export const ADD_MESSAGE = "ADD_MESSAGE";
export const DELETE_MESSAGE = "DELETE_MESSAGE";
export const CLEAR_MESSAGES = "CLEAR_MESSAGES";

// Code reducers
export const CREATE_CODE = "CREATE_CODE";
export const READ_CODE_ALL = "READ_CODE_ALL";
export const READ_CODE = "READ_CODE";
export const UPDATE_CODE = "UPDATE_CODE";
export const DELETE_CODE = "DELETE_CODE";
export const WRITE_LOADING_CODE = "WRITE_LOADING_CODE";
export const READ_LOADING_CODE = "READ_LOADING_CODE";
export const SET_CODE_ERROR = "SET_CODE_ERROR";

// Field reducers
export const CREATE_FIELD = "CREATE_FIELD";
export const READ_FIELD_ALL = "READ_FIELD_ALL";
export const READ_FIELD = "READ_FIELD";
export const UPDATE_FIELD = "UPDATE_FIELD";
export const DELETE_FIELD = "DELETE_FIELD";
export const WRITE_LOADING_FIELD = "WRITE_LOADING_FIELD";
export const READ_LOADING_FIELD = "READ_LOADING_FIELD";
export const SET_FIELD_ERROR = "SET_FIELD_ERROR";

// Obj reducers
export const CREATE_OBJ = "CREATE_OBJ";
export const READ_OBJ_ALL = "READ_OBJ_ALL";
export const READ_OBJ = "READ_OBJ";
export const UPDATE_OBJ = "UPDATE_OBJ";
export const DELETE_OBJ = "DELETE_OBJ";
export const WRITE_LOADING_OBJ = "WRITE_LOADING_OBJ";
export const READ_LOADING_OBJ = "READ_LOADING_OBJ";
export const SET_OBJ_ERROR = "SET_OBJ_ERROR";

// App reducers
export const CREATE_APP = "CREATE_APP";
export const READ_APP_ALL = "READ_APP_ALL";
export const READ_APP = "READ_APP";
export const UPDATE_APP = "UPDATE_APP";
export const DELETE_APP = "DELETE_APP";
export const WRITE_LOADING_APP = "WRITE_LOADING_APP";
export const READ_LOADING_APP = "READ_LOADING_APP";
export const SET_APP_ERROR = "SET_APP_ERROR";
